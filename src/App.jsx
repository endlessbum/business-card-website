...
function VisitorCounter() {
  ...
  return (
    <div
      className="hero-views"
      title="уникальные посетители / unique visitors"
      aria-label={`уникальных посетителей: ${count}`}
    >
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
        <path d="M1 12s4-7.5 11-7.5S23 12 23 12s-4 7.5-11 7.5S1 12 1 12Z" />
        <circle cx="12" cy="12" r="3" />
      </svg>
      <span className="hero-views-count">{count.toLocaleString('ru-RU')}</span>
      <span className="sr-only">уникальных посетителей: {count}</span>
    </div>
  )
}
...
function Natal() {
  return (
    <section id="natal" className="section">
      <SectionHead title={natal.title} titleRu={natal.titleRu} index={6} />
      <div className="section-body" data-reveal>
        {natal.text.map((p, i) => (
          <p key={i} className="lead">
            {p.ru}
...
function Facts() {
  return (
    <section id="facts" className="section">
      <SectionHead title={facts.title} titleRu={facts.titleRu} index={7} />
      <ul className="facts" data-reveal>
        {facts.items.map((f, i) => (
          <li key={i} className="fact">
            <span className="fact-mark">[ {String(i + 1).padStart(2, '0')} ]</span>
...
function Goals() {
  return (
    <section id="goals" className="section">
      <SectionHead title={goals.title} titleRu={goals.titleRu} index={8} />
...
function Socials() {
  return (
    <section id="socials" className="section">
      <SectionHead title={socialsTitle.title} titleRu={socialsTitle.titleRu} index={9} />
